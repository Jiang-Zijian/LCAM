...
def gradient_density(y,where,var,phip,lbd,alpha,real=False):
    ...
    def kI():        
        return np.average((np.conjugate(lbd)*lbd)/(np.conjugate(lbd*GG-1)*(lbd*GG-1)+UV*(np.conjugate(lbd)*lbd)))
    def kI_part():
        return (np.conjugate(lbd)*lbd)/(np.conjugate(lbd*GG-1)*(lbd*GG-1)+UV*(np.conjugate(lbd)*lbd))**2
    def kIp(): 
        return np.average((lbd)/(np.conjugate(lbd*GG-1)*(lbd*GG-1)+UV*(np.conjugate(lbd)*lbd)))
    def kIp_part():
        return (lbd)/(np.conjugate(lbd*GG-1)*(lbd*GG-1)+UV*(np.conjugate(lbd)*lbd))**2
    def kI1_():    
        """
        d kI/d G^\prime
        """
        return np.average(-1.0*kI_part()*(lbd*np.conjugate(lbd*GG-1)))
    def kI2_():    
        """
        d kI/d (G^\prime)^\dagger
        """
    ...
    def kI3_():    
        ...
        d kI^\prime/d G^\prime
        """
        return np.average(-1.0*kIp_part()*(lbd*np.conjugate(lbd*GG-1)))
    def kI4_():    
        """
        d kI^\prime /d (G^\prime)^\dagger
        """
    ...
    def kI11_():
        ...
        d (kI) /d u^\prime v
        """
        return np.average(-1.0*kI_part()*(1.0*np.conjugate(lbd)*lbd))
    def kI22_():
        """
        d (kI^\prime) /d u^\prime v
        """
        return np.average(-1.0*kIp_part()*(1.0*np.conjugate(lbd)*lbd))
    def kI33_():
